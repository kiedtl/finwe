            ...
            return switch (self) {
                ...
                .Of => |of| b: {
                    ...
                    for (new.def.Struct.fields.items, 0..) |*field, i| {
                        ...
                        UserType.checkStructField(field.type, i == new.def.Struct.fields.items.len - 1);
                    }
                    if (calculate_offset) {
                        var offset: u16 = 0;
                        for (new.def.Struct.fields.items, 0..) |*field, i| {
...
pub const ASTNode = struct {
    ...
    pub const Type = union(enum) {
        ...
        pub fn format(self: @This(), comptime f: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
            ...
            switch (self) {
                ...
                .Wild => try writer.print("Wild<elided>", .{}),
    ...
    pub const Cast = struct {
        ret: bool = false,
        // FIXME: why not just allocate ArrayList? what was I thinking?
    ...
    pub fn deepclone(self: @This(), parent: ?*Decl, program: *Program) @This() {
        ...
        switch (new) {
            ...
            .Cond => |*c| {
                ...
                c.branches = @TypeOf(c.branches).init(program.alloc);
                for (self.node.Cond.branches.items) |br| {
                    c.branches.append(.{
                        .body = _deepcloneASTList(br.body, parent, program),
                        .cond = _deepcloneASTList(br.cond, parent, program),
                        .cond_srcloc = br.cond_srcloc,
                        .body_srcloc = br.body_srcloc,
                        .cond_prep = br.cond_prep,
                        .cond_arity = br.cond_arity,
                    }) catch unreachable;
                }
            },
            .Loop => {
    ...
    pub fn deinit(self: *@This(), recursed_ctr: usize, program: *Program) void {
        const _freeList = struct {
            pub fn f(lst: ASTNodeList, r: usize, p: *Program) void {
                var itr = lst.iterator();
                while (itr.next()) |item| {
                    item.deinit(r + 1, p);
                }
        ...
        // stdout.writeBytesNTimes("- ", recursed_ctr) catch unreachable;
        ...
        switch (self.node) {
            ...
            .When => |w| {
                ...
                if (w.nah) |n| _freeList(n, recursed_ctr + 1, program);
            },
            .Cond => |c| {
            ...
            .Loop => |l| {
                ...
                switch (l.loop) {
                    .Until => |u| _freeList(u.cond, recursed_ctr + 1, program),
...
pub const UserType = struct {
    ...
    pub fn deepclone(self: @This()) @This() {
        var new = self;
        new.scope = new.scope.shallowclone();
        switch (new.def) {
